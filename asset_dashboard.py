...
def create_age_distribution_chart(df):
    """Create chart untuk asset age distribution"""
    if "Asset Age" not in df.columns or df["Asset Age"].sum() == 0:
        return None
    
    df_temp = df[df["Asset Age"] > 0].copy()
    age_counts = df_temp["Asset Age"].value_counts().sort_index()
    
    fig = px.bar(
        x=age_counts.index,
        y=age_counts.values,
        title="Asset Age Distribution",
        labels={'x': 'Age (Years)', 'y': 'Number of Assets'},
